;(function($){
	$(function(){
		...
		//问题1继续
		$("#question1 .btn-go-on").on('click',function(){
			goOn($('#question1,#hover,.ques1,#hover2'));
		});
		
		//问题2继续
		$("#question2 .btn-go-on").on('click',function(){
			goOn($('#question2,#hover,.ques2,#hover2'));
		});
		
		//问题3继续
		$("#question3 .btn-go-on").on('click',function(){
			goOn($('#question3,#hover,.ques3,#hover2'));
		});
		
		//问题4继续
		$("#question4 .btn-go-on").on('click',function(){
			goOn($('#question4,#hover,.ques4,#hover2'));
		});
		
		//从新玩
		$("#fail .btn-jx").on('click',function(){
			gameReset();
			$("#fail").hide();
			//timer = setInterval(countdown,1000);
		});
		...
		//返回主界面
		$("#fail .btn-back,#success .btn-back").on('click',function(){
			gameReset();
			$(this).parent().hide();
			$("#main").hide();
			$("#index").show();
